...
def _get_concat_nodes(mod):
    ...
    def visit(node):
        tuple_pattern = is_tuple([wildcard(),wildcard()])
    ...
    return concatenate_nodes, parent_nodes
...
class UnetCallback(DFPatternCallback):
    ...
    def callback(self, pre, post, node_map):
        if node_map[self.x][0] in self.match_node:
            print("pat 1")
...
def quantize(mod, quantization_level):
    # get concatenate and its parent (2nd) node
    upc = UnetPreProcessCallback()
    out = rewrite(upc, mod['main'])

    # Deprecated : This job should be done in post
    # Foward intermediate node to output for liveness
    if quantization_level == 0:
        ...
        # cocat_nodes, concat_parent_nodes = _get_concat_nodes(mod['main'])
        concat_nodes = upc.match_node2
        concat_parent_nodes = upc.match_node
        print(len(concat_nodes))
        print(len(concat_parent_nodes))
        print(concat_nodes)
        uc = UnetCallback(match_node=concat_nodes)
        ...
        print(len(concat_nodes))
        print(len(concat_parent_nodes))
        uc2 = UnetCallback2(concat_parent_nodes)
        out = rewrite(uc2, out)
